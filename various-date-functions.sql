...
-- maximum address id finding
select max(address_id) from customer;


-- second highest address id
select max(address_id) from customer where address_id<(select max(address_id) from customer);


-- In SQL, when sorting data, there are only two sorting directions available with ORDER BY:
-- | Keyword | Meaning                                      | Default     |
-- | ------- | -------------------------------------------- | ----------- |
-- | `ASC`   | Ascending order (smallest to largest / A–Z)  | ✅ (default) |
-- | `DESC`  | Descending order (largest to smallest / Z–A) | ❌           |

-- finding second max
SELECT address_id FROM customer ORDER BY address_id DESC LIMIT 3 offset 1;

--  top second address-id
SELECT address_id FROM address ORDER BY address_id DESC LIMIT 1 offset 1;

-- top 3 address_d
SELECT address_id FROM address ORDER BY address_id DESC LIMIT 3 ;

-- to find current date
select now();   -- "yyyy-mm-dd hh-mm-ss"
select curdate();
select sysdate();
select second("2025-07-25 10:53:47");
select minute("2025-07-25 10:53:47");
select hour("2025-07-25 10:53:47");
select day("2025-07-25 10:53:47");
select month("2025-07-25 10:53:47");
select year("2025-07-25 10:53:47");
select time("2025-07-25 10:53:47");
...
select week(now());  -- count the weeks passed in a year
select weekofyear(now());
select quarter(now());
select dayofyear(now());
select weekday(now());       # start from zero, start from monday
select dayofweek(now());     # start from 1, start from sunday


-- 1. Add 5 days to today:
SELECT DATE_ADD(NOW(), INTERVAL 5 DAY);

-- 2. Add 2 months to a specific date:
SELECT DATE_ADD('2025-01-10', INTERVAL 2 MONTH);

-- subtract from specific data
select datediff(now(), date_sub(now(), interval 2 month));

-- string to date
select str_to_date("2025-07-25 11:23:50", "%d-%m-%y") from customer;
select date_add(create_date, interval 10 month) from customer;

-- to find the last day of the month
SELECT LAST_DAY("2025-07-25");


-- ✅ MAKETIME(h, m, s) → Creates a time value like 'hh:mm:ss'
-- ✅ MAKEDATE(year, day_of_year) → Creates a date from year and day number (1–366)
select maketime(20,30,0);
select makedate(2025,200);

select * from payment;
select max(amount) from payment; -- max-11.99
select min(amount) from payment;  -- 0.0
 select avg(amount) from payment;
 
 
select amount, if(amount>4.2,"higher","lower") as category from payment;
-- It selects the amount from the payment table.
-- Then, it uses the IF() function to create a new column called category.

select amount,
case
    when amount < 3.0 then "lower"
    when amount between 3.0 and 5.0 then "medium"
    when amount > 5.0 then "higher"
    else "not decided"
    end as filter
    from payment;
